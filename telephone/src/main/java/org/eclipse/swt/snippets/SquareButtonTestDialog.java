...
public class SquareButtonTestDialog extends ApplicationWindow {
	...
	protected Control createContents(Composite parent) {
		...
		customButton.setText("Hold");
		
		
		// a small background image that can be tiled
		Image image = new Image(getShell().getDisplay(), SquareButtonTestDialog.class.getResource(IMG1_PATH).getFile());
		...
		image = new Image(getShell().getDisplay(), SquareButtonTestDialog.class.getResource(IMG2_PATH).getFile());
	...
	public SquareButton getCustomButton (Composite parent) {
		...
		button.setInnerMarginHeight(6);
		button.setInnerMarginWidth(18);
		button.setRoundedCorners(true);
		
		FontData[] fontData = JFaceResources.getDialogFont().getFontData();
		Font font = getSavedFont(fontData[0].getName(), fontData[0].getHeight(), fontData[0].getStyle());
