...
import net.cloudengine.client.main.SWTGraphicsUtils;
...
//	private List<String> autorizedLogin;
	...
	private void buildShell() {
		...
		this.shell.setText("Ingreso al sistema");
	...
	private Image createDefaultImage(final int w, final int h) {
		...
		final Point textSize = gc.stringExtent("Ingreso al sistema");
		gc.drawString("Ingreso al sistema", (int) (w * .05f), (h - textSize.y) / 2, true);
	...
	private void buildDescription() {
		...
		final Font bold = SWTGraphicsUtils.buildFontFrom(label, SWT.BOLD);
		label.setFont(bold);
		label.addListener(SWT.Dispose, new Listener() {

			@Override
			public void handleEvent(final Event event) {
				SWTGraphicsUtils.dispose(bold);
			}
		});
...
//		} else {
			...
			text.setLayoutData(new GridData(GridData.FILL, GridData.END, true,
					false, 3, 1));
			text.setFocus();
...
//		}
	...
	private void buildRememberPassword() {
		...
		checkbox.setText("Recordarme en esta computadora");
	...
	private void openShell() {
		...
		Image small = new ImgBundle().getImageDescriptor("login-16.png").createImage();
		Image medium = new ImgBundle().getImageDescriptor("login-32.png").createImage();
		Image large = new ImgBundle().getImageDescriptor("login-48.png").createImage();
		shell.setImages(new Image[] { small, medium, large });
		
		
		this.shell.setDefaultButton(this.buttonOk);
		this.shell.pack();
		SWTGraphicsUtils.centerShell(this.shell);		
		this.shell.open();
...
//	}
	...
	/**
	 * @param displayRememberPassword
	 *            if <code>true</code>, the checkbox "remember the password" is
	 *            displayed
	 */
