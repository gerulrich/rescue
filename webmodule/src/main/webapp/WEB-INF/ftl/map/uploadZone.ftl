	...
	$(document).ready(function() {
		...
		$("#target").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
            		type: 'POST',
            		url: '<@spring.url '/shp/upload/zone/${file.id}'/>',
            		dataType: 'json',
            		data: d,
...
<#macro body>
	...
	<@page.section>
		<@page.half>
			<@page.box title="Subir shapefile como capa de zonas">
				<form id="target" method="post">
					...
					<@form.row label="Campo Nombre">
						<@form.simpleSelect "nameField" fields/> 
					</@form.row>
				
					<@form.row label="Tipo">
						<input type="type" name="type" class='{validate:{required:true, messages:{required:"Ingrese una valor para el campo tipo"}}}'/>
					</@form.row>
